		...
		try 
		{
			...
			while ((line = inputfile.readLine()) != null)  
			{
				...
				else if (line.contains("\t")) //Annotation
	        	{
	        		...
	        		if(anno.length>=6)
	        		{
	        			...
	        			if(TrainTest.equals("Train"))
	        			{
		        			...
		        			if(m1.find())
		        			{
		        				...
		        				{
		        					String patt="^(.*?)("+M[i]+")(.*?)$";	        					Pattern ptmp = Pattern.compile(patt);
		        					Matcher mtmp = ptmp.matcher(mention_tmp);
		        					if(mtmp.find())
		        					{
		        						for(int j=mtmp.group(1).length();j<(mtmp.group(1).length()+mtmp.group(2).length());j++)
		        				...
		        				for(int i=0;i<F.length;i++)
		        				{
		        					...
		        					String patt="^(.*?)("+F[i]+")(.*?)$";	        					Pattern ptmp = Pattern.compile(patt);
		        					Matcher mtmp = ptmp.matcher(mention_tmp);
		        					if(mtmp.find())
		        					{
		        						for(int j=mtmp.group(1).length();j<(mtmp.group(1).length()+mtmp.group(2).length());j++)
		        				...
		        				{
		        					String patt="^(.*?)("+S[i]+")(.*?)$";	        					Pattern ptmp = Pattern.compile(patt);
		        					Matcher mtmp = ptmp.matcher(mention_tmp);
		        					if(mtmp.find())
		        					{
		        						for(int j=mtmp.group(1).length();j<(mtmp.group(1).length()+mtmp.group(2).length());j++)
		        			...
		        			else if(m5.find())
		        			{
		        				...
		        				for(int i=0;i<D.length;i++)
		        				{
		        					...
		        					String patt="^(.*?)("+D[i]+")(.*?)$";	        					Pattern ptmp = Pattern.compile(patt);
		        					Matcher mtmp = ptmp.matcher(mention_tmp);
		        					if(mtmp.find())
		        					{
		        						for(int j=mtmp.group(1).length();j<(mtmp.group(1).length()+mtmp.group(2).length());j++)
	        		...
	        		{
	        			System.out.println("Error! annotation column is less than 6.");
	        		}
	        	}
